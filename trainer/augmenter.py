...
class Augmenter:
    ...
    def augment(self, img):
        #img = torchvision.transforms.RandomApply([torchvision.transforms.RandomResizedCrop(
        #                size=512,
        #                scale=(0.08, 1.0),
        #                ratio=(0.75, 1.0 / 0.75),
        #                interpolation=self.rz_dict[list(self.rz_dict.keys())[sample_discrete([0,1,2,3])]])],p=0.2)(img)
        if self.use_nested:
            iterations = sample_discrete([1,2,3])
        else:
            iterations = 1
        toTensor = torchvision.transforms.ToTensor()
        ...
        for iteration in range(iterations):
            ...
            if operations[cidx] != 'no_op':
                #post_proc = self.process['compress'](img)
                post_proc = self.process[operations[cidx]](post_proc)
            else:
                post_proc = post_proc
        final = torchvision.transforms.Resize((self.final_rez, self.final_rez), interpolation=Image.BICUBIC)
        return toTensor(final(post_proc))
